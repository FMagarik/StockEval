...
namespace DDS.Models {
    public class Parser {
        enum Símbolos {
            ...
            LITERAL = StreamTokenizer.TT_NUMBER
        }
        ...
        private double Factor() {
            ...
            if (EsToken(Símbolos.NOMBRE)) {
                ...
                if (m != null) {
                    valor = m.CumpleCondiciones(cuentas) ? 1 : 0;
                    if (!m.parseosVálidos) GenError("error por nombre no encontrado");
                } else {
                    Indicador i = Indicador.Get(id);
                    if (i != null) {
                        valor = i.CalcularValor(cuentas);
                        if (!i.parser.EsVálido()) GenError("error por nombre no encontrado");
                    } else {
            ...
            } else if (Aceptar(Símbolos.LITERAL)) {
                valor = tokens.NumberValue;
            } else {
